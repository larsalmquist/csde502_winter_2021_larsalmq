...
The table supplied above is confusing, as having a value of `6` for the 10th row is logically incompatible with the `white` variable. Additionally there is no codebook indicator for a value of `8`. 
...
## brute force drop last row for white/AI/other?
## transpose back as needed (see HTML on course site)
# rowsums vs. colsums


attributes(dat$h1kq1b)$labels %>% 
  t() %>% 
  t() %>% 
  data.frame()

dat %<>%
  mutate(multi_sum = colSums(.))

ans_loop %<>%
    # calculate the rowSums
    mutate(h1kqNa_sum = rowSums(.))

multirace_sum = colSums(.)
(h1kqNa_sum = rowSums(.))
